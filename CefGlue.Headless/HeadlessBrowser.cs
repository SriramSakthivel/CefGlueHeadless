...
public class HeadlessBrowser : IDisposable
{
    ...
    private readonly List<string> urlsToTrack = new List<string>();
    private readonly object locker = new object();

    public bool ShouldTrackRequest(CefRequest request)
    {
        return false;
        lock (locker)
        {
            return urlsToTrack.Contains(request.Url);
        }
    }
    ...
    /// </summary>
    /// <param name="url">A fully qualified URL</param>
    public Task<NavigationStatus> NavigateAsync(Uri url)
    {
        AssertNotInitialized();
        lock (locker)
        {
            urlsToTrack.Add(url.ToString());
        }
        var tcs = new TaskCompletionSource<NavigationStatus>();
        ...
        szd = (b, httpStatus) =>
        {
            ...
            lock (locker)
            {
                urlsToTrack.Remove(url.ToString());
            }
            tcs.TrySetResult(new NavigationStatus(url.ToString(), httpStatus));
        };
        client.LoadingEnded += szd;
        Navigate(url);
...
{
    public NavigationStatus(string uri, int httpStatusCode)
    {
        Uri = uri;
...
public delegate void BrowserFrameReceivedDelegate(object image);
public delegate void BrowserInitializedDelegate(HeadlessBrowser browser);
public delegate void BrowserLoadEndedDelegate(HeadlessBrowser browser, int httpStatusCode);
...
public delegate void BrowserSizeChangedDelegate(HeadlessBrowser browser, int width, int height);